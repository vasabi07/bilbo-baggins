...
def execute_tool(state: AgentState) -> AgentState:
    """Call the required tool (e.g., Tavily search)."""
    query = state["messages"][-1]["content"]
...
def generate_response(state: AgentState) -> MessagesState:
    """Generate final response using LLM with tool results (if any)."""
    query = state["messages"][-1]["content"]
...
if __name__ == "__main__":
    ...
    response = graph.invoke({"messages": [Message(type="human", content=user_query)]})
